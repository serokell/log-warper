...
import           System.Log.Handler        (LogHandler (getTag), close)
...
logMCond :: String -> Priority -> String -> (String -> Bool) -> IO ()
logMCond logname pri msg cond = do
    l <- getLogger logname
    logLCond l pri msg cond
...
logLCond :: Logger -> Priority -> String -> (String -> Bool) -> IO ()
logLCond l pri msg = handle l (pri, msg)

-- | Handle a log request.
handle :: Logger -> LogRecord -> (String -> Bool) -> IO ()
handle l lrecord@(pri, msg) handlerFilter = do
    lp <- getLoggerPriority (name l)
    if pri >= lp
      then do
  ...
  where
    ...
    -- Get the priority we should use. Find the first logger in the
    -- tree, starting here, with a set priority. If even root doesn't
    -- have one, assume DEBUG.
    getLoggerPriority :: String -> IO Priority
    getLoggerPriority name = do
    ...
    callHandler lr loggername (HandlerT x) =
        ...
        System.Log.Handler.handle x lr loggername
